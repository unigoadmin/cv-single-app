...
@Component({
  selector: 'cv-edit-root-user',
  templateUrl: './edit-root-user.component.html',
  styleUrls: ['./edit-root-user.component.scss']
})
export class EditRootUserComponent implements OnInit {
  @Input() companyId:number;
  @Input() rootUserId:string;
  form = this.fb.group({
    FirstName: null,
    LastName: null,
    PrimaryEmail: null,
  ...
  ngOnInit() {
     ...
     if (this.loginUser) {
      this.employerId = this.rootUserId;
 ...
 getInternalUserRolesByUserId() {
  forkJoin([
    this._accountService.getInternalUserProfile(this.rootUserId),
    this._commonService.GetAllTimeZones(),
    this._commonService.getStates(),
    this._commonService.getKeywords(this.companyId),
