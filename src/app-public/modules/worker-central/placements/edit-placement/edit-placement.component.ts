...
export class EditPlacementComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.loginUser) {
      ...
      if (this.def_placement.Id > 0 && this.def_placement.Mode == 'Edit') {
  ...
  OnRequirementChange(type) {
    ...
    if (type === 2) {
      ...
      //this.DynamicValidation('EndClient');
    ...
    if (!isNullOrUndefined(this.confirmationCandidate.Email)) {
      ...
      if (response.IsError == false) {
        ...
        if (this.existingCandidate.CandidateID > 0) {
          this.dialog.open(CandidateExistsComponent, { width: '60%', panelClass: "dialog-class", disableClose: true, data: this.existingCandidate }).afterClosed().subscribe((confirmation) => {
            if (confirmation) {
  ...
  CandidateselectionChange(event) {
    ...
    //this.candidateEmpType = event.value;
    //this.placementCandidate.EmploymentType = event.value;
  }
  ...
  GetEndClientInternalData(event) {
    if (this.endClientAccount.PlacementAccountMappingID != 0) {
      this.endClientAccount = this.UpdateAccountandContact(event, this.endClientAccount);
    } else {
  ...
  GetPrimeVendorData(event) {
    if (this.primeVClientAccount.PlacementAccountMappingID != 0) {
      this.primeVClientAccount = this.UpdateAccountandContact(event, this.primeVClientAccount);
    }
    else {
  ...
  GetMSPVendorData(event) {
    if (this.mspClientAccount.PlacementAccountMappingID != 0) {
      this.mspClientAccount = this.UpdateAccountandContact(event, this.mspClientAccount);
    } else {
  ...
  GetIPVendorData(event) {
    if (this.ipClientAccount.PlacementAccountMappingID != 0) {
      this.ipClientAccount = this.UpdateAccountandContact(event, this.ipClientAccount);
    } else {
  ...
  GetSubpvVendorData(event) {
    if (this.subPVClientAccount.PlacementAccountMappingID != 0) {
      this.subPVClientAccount = this.UpdateAccountandContact(event, this.subPVClientAccount);
    } else {
  ...
  GetRefVendorData(event) {
    if (this.refClientAccount.PlacementAccountMappingID != 0) {
      this.refClientAccount = this.UpdateAccountandContact(event, this.refClientAccount);
    } else {
  ...
  GetPlacement() {
    this._service.GetPlacementByIdForEdit(this.loginUser.Company.Id, this.def_placement.Id).subscribe(response => {
      if (response.IsError) {
        this._alertService.error(response.ErrorMessage);
      } else {
        this.confirmation = response.Data;
        this.InitialEmailId = this.confirmation.Candidate.Email;
        this.SelectedPOC = this.confirmation.PlacementPOC;
        ...
        if (this.confirmation && this.confirmation.Candidate !== null) {
