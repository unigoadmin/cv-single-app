...
@Component({
  selector: 'cv-archive-assign',
  templateUrl: './archive-assign.component.html',
  styleUrls: ['./archive-assign.component.scss']
})
export class ArchiveAssignComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public def_applicants: any,
    private dialogRef: MatDialogRef<ArchiveAssignComponent>,
  ...
  AssignSingleResponse() {
    if (this.SelectedAssigness.length > 0) {
      ...
      const applicants = {
        ...
        ArchiveId:this.def_applicants.responseId
      }
      
      this.jobCentralService.AssignSingleArchieveApplicants(applicants).subscribe(response => {
        if (!response.IsError) {
