...
@Component({
  selector: 'cv-sub-prime-vendor',
  templateUrl: './sub-prime-vendor.component.html',
  styleUrls: ['./sub-prime-vendor.component.scss'],
  providers: [AccountTypes,VendorLayerService]

})
export class SubPrimeVendorComponent implements OnInit {
  icClose=icClose;
  @Input('subPVClientAccount') subPVClientAccount: any;
  @Input('DisplayBilling') DisplayBilling: Boolean;
  @Output() out_subPVClientAccount = new EventEmitter<any>();
  @Output() out_removesubPVAccount = new EventEmitter<any>();
  subPrimeVendorFromGroup : FormGroup;
  EndClientList: AccountMaster[] = [];
  accountMaster: AccountMaster[] = [];
  subPrimeVendorControl: FormControl = new FormControl();
  subpvOptions: Observable<AccountMaster[]>;
  loginUser: LoginUser;
  ...
  constructor(private fb: FormBuilder,
    ...
    private cdRef: ChangeDetectorRef) { 
      this.subPrimeVendorFromGroup = this.fb.group({
        subpvEndclientName:[null,[Validators.required]],
      ...
      this.subPrimeVendorFromGroup.valueChanges.subscribe(() => {
        this.formValidityChanged.emit(this.subPrimeVendorFromGroup.valid);
      });
    }
  ...
  PepareAccountTypeAccountList() {debugger;
    ...
    this.subpvOptions = this.subPrimeVendorControl.valueChanges.pipe(startWith(''), map(val => this.endclientfilter(val)));
  }
  ...
  SeelectedAccount(event, type) {
    ...
    this.subPVClientAccount = this.GetAccountandContact(subv); 
    this.out_subPVClientAccount.emit(this.subPVClientAccount);
  }
  ...
  onRemoveClick(event){
    this.out_removesubPVAccount.emit(event);
  }

  SelectBillingAccount(event,type){
    if (event.checked){
      this.subPVClientAccount.Isbilling = true;
      ...
      this.subPVClientAccount.Isbilling = false;
      this.subPVClientAccount.BillingPOC = null;
      this.subPVClientAccount.ImmigrationPOC = null;
    }
  }

  GetBllingData(event){
   this.subPVClientAccount  = this.DeepCopyForObject(event);
   console.log(this.subPVClientAccount);
   this.out_subPVClientAccount.emit(this.subPVClientAccount);
  }
