...
export class SuperAdminPanelSecurityAuthGuard implements CanActivate {
        ...
        if (this.authenticationService.isLoggedIn() && loginUser!=null)
        {
            ...
            if (permissions != null) {
                ...
                if (loginUser != null && permissions.role != null && permissions.module != null) {
                    if (permissions.role.includes(loginUser.Role) && this.ValidateUserRole(loginUser.Role)) {
                        return true;
                    }
                }
                UnAuthAccess = true;
    ...
    ValidateUserRole(RoleName:string){
        let IsValid: boolean = false;
        if(RoleName == "admin"){
            return true;
        }
